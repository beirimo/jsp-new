...
public class KakaoServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		switch(request.getServletPath()){
			case "/kakao/book":
				request.setAttribute("pageName", "/kakao/book.jsp"); //케이스이거면 페이지네임에 /kakao/book을 넣어줘라 
				System.out.println("/kakao/book으로 갈까요?");
				break;
			case "/kakao/local":
				request.setAttribute("pageName", "/kakao/local.jsp");  //케이스이거면 페이지네임에 /kakao/book을 넣어줘라 
