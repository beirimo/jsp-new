	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		case "/bbs/total":
			...
			case "/bbs/list.json":
				...
				int size=request.getParameter("size") != null ? Integer.parseInt(request.getParameter("size")) : 3; //사이지를 지정하면 그거 아니면 디폴트값10이다. 확인해볼거면 // http://localhost:8080/bbs/list.json?page=1&size=3
				Gson gson = new Gson();
